...
const Reel: React.FC<ReelProps> = ({ symbols, spinning, delay, winHighlightRows }) => {
  return (
    <div className="relative w-full h-full border-r border-[#C8AA6E]/10 last:border-r-0 bg-[#010A13]/80 backdrop-blur-md">
      ...
      >
        {spinning ? (
          ...
          <div className="flex flex-col h-full justify-around opacity-60">
             <div className="h-[33%] bg-gradient-to-t from-[#C8AA6E]/20 to-transparent rounded-xl mx-2"></div>
             <div className="h-[33%] bg-gradient-to-t from-[#0AC8B9]/20 to-transparent rounded-xl mx-2"></div>
             <div className="h-[33%] bg-gradient-to-t from-[#C8AA6E]/20 to-transparent rounded-xl mx-2"></div>
          </div>
        ) : (
          symbols.map((symbolId, rowIndex) => {
            ...
            return (
              <div
                key={rowIndex}
                className={`relative flex items-center justify-center h-[33.33%] transition-all duration-300 group
